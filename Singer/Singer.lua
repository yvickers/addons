...
default = {
    ...
    troubadour=true,
    debuffing=false,
    recast={song={min=30,max=55},buff={min=25,max=50}},
    active=true,
    timers=true,
...
setting = T{
    ...
    dummy = L{"Army's Paeon",},
    songs = L{"Honor March","Victory March","Valor Minuet V",},
    song = {},
    playlist = T{
        clear = L{}
    },
...
function do_stuff()
    ...
    if counter >= del then
		...
		if not JA_WS_lock and play.status == 1 then
			...
			if (get.eye_sight(windower.ffxi.get_mob_by_target('me'),targ) and play.vitals.tp >= goal_tp and 
                ...
                math.sqrt(targ.distance) <= 4) then
